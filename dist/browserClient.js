...
let page = null;
const PROFILE_DIR = process.env.BROWSER_PROFILE_DIR || path.resolve("data/browser-profile");
...
export async function fetchAvailabilityInBrowser({ storeNumber, slotType, startDate, maxNumberOfDays = 42, lineOfBusiness = "OPTICAL", }) {
    ...
    if (looksBlocked(currentUrl, currentTitle)) {
        throw new Error("Manual verification still required in noVNC before availability can be fetched.");
    }
    if (!/specsavers\.co\.uk/i.test(currentUrl)) {
        throw new Error(`Browser is not on the Specsavers site. Current URL: ${currentUrl}`);
    }
    const result = await page.evaluate(async ({ storeNumber, slotType, startDate, maxNumberOfDays, lineOfBusiness }) => {
        const query = `
